...
export default function UsersManager({ users }: { users: ManagedUser[] }) {
  ...
  return <div className="space-y-6">
    <section className="overflow-hidden rounded-2xl border border-border bg-card">
      ...
      <ul className="divide-y divide-border">{users.map((user) => <li key={user.id} className="flex flex-wrap items-center gap-3 px-5 py-4"><span className="flex size-9 items-center justify-center rounded-full bg-primary/10 text-xs font-semibold text-primary">{user.initials}</span><div className="min-w-0 flex-1"><p className="truncate text-[13px] font-medium">{user.name}</p><p className="truncate text-xs text-muted-foreground">@{user.username}</p></div><span className="rounded-full bg-[#16B1FF]/15 px-2.5 py-1 text-xs font-medium text-[#0a6ea3] dark:text-[#7fd4ff]">{user.role === "PM" ? "PM" : "Viewer"}</span><div className="flex gap-1"><button type="button" aria-label={`Edit ${user.name}`} onClick={() => { setEditing(user); setCreating(false); setError(""); }} className="flex size-8 items-center justify-center rounded-full bg-[#FFB400]/15 text-[#a66f00] transition-colors hover:bg-[#FFB400]/25 dark:text-[#ffd166]"><Pencil className="size-4" /></button><button type="button" aria-label={`Delete ${user.name}`} disabled={pending} onClick={() => remove(user)} className="flex size-8 items-center justify-center rounded-full text-destructive hover:bg-destructive/10"><Trash2 className="size-4" /></button></div></li>)}{!users.length && <li className="px-5 py-8 text-center text-[13px] text-muted-foreground">No users yet.</li>}</ul>
    </section>
